...
def safety_stock(
    z, std_dev, lead_time, review_period=0, label="Safety stock", suffix=""
):
    """
...
def service_level_safety_stock(
    ...
    lead_time,
    service_level,
    label="Service level safety stock",
    suffix="",
):
    """
...
def variable_lead_time_safety_stock(
    z,
    mean_demand,
    std_demand,
    mean_lead_time,
    std_lead_time,
    review_period=0,
    label="Variable lead time safety stock",
    suffix="",
):
    """
...
def calculate_luc_criterion(
    t, z, setup_cost, holding_cost, demands, label="LUC criterion", suffix=""
):
    """
...
def calculate_sm_criterion(
    t, z, setup_cost, holding_cost, demands, label="Silver-Meal criterion", suffix=""
):
    """
...
def make_lot_sizing_decision(
    func_cost,
    num_periods,
    setup_cost,
    holding_cost,
    demands,
    label="Lot sizing decision",
    suffix="",
):
    """
...
def guaranteed_service_model(
    lead_times, holding_costs, demand_mean, std, service_level, candidates=None
):
    """
...
def clark_scarf_model(lead_times, mu, sigma, holding_costs, penalty_cost):
    ...
    opt_s2 = norm.ppf(
        critical_ratio_s2,
        mu * (lead_times[1] + 1),
        sigma * math.sqrt(lead_times[1] + 1),
    )
    ...
    def func_s1(s1):
        ...
        integral = integrate.quad(
            lambda d: norm.cdf(
                s1 - d, mu * lead_times[0], sigma * math.sqrt(lead_times[0])
            )
            * norm.pdf(
                d, mu * (lead_times[1] + 1), sigma * math.sqrt(lead_times[1] + 1)
            ),
            0,
            opt_s2,
        )[0]
...
def metric_model(demand_rate, penalty_cost, holding_costs, lead_times):
    ...
    def func_exp_total_cost(base_stock_w, base_stock_r):
        ...
        exp_total_cost = holding_costs[0] * exp_leftover_w + func_exp_cost_r(
